  ...
  <div class="ces-search">
    <el-form :size="size" :inline='false' :label-width="labelWidth">
      <el-form-item v-for="item in searchForm" :label="item.label" :key="item.prop">
        <!-- 输入框 -->
        <el-input v-if="item.type==='Input'" v-model="searchData[item.prop]" size="small"></el-input>
        <!-- 下拉框 -->
        <el-select
        ...
        <!-- 按钮 -->
        <el-button v-if="item.type==='Button'" :type="item.BtnType" :size="item.size || size" @click="item.handle()">{{item.label}}</el-button>
      </el-form-item>
    </el-form>
